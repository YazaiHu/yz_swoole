...
static void client_onReceive(swClient *cli, char *data, uint32_t length)
{
    ...
    {
        swWarn("swoole_client object have not receive callback.");
        goto free_zdata;
    }
...
static void client_onClose(swClient *cli)
{
    ...
    char* pHost;
    int nPort;
    swClient_inet_peer(cli, &pHost, &nPort);
    swWarn("swoole_client::client_onClose [cli=%p, ip=%s, port=%d, fd=%d]", cli, pHost, nPort, cli->socket->fd);
    sw_free(pHost)
#endif
...
static void client_onError(swClient *cli)
{
    ...
    int nPort;
    swClient_inet_peer(cli, &pHost, &nPort);
    swWarn("swoole_client::client_onError [cli=%p, ip=%s, port=%d, fd=%d]", cli, pHost, nPort, cli->socket->fd);
    sw_free(pHost)
#endif
#if PHP_MAJOR_VERSION < 7
    TSRMLS_FETCH_FROM_CTX(sw_thread_ctx ? sw_thread_ctx : NULL);
...
void php_swoole_at_shutdown(char *function)
{
    ...
    if (!register_user_shutdown_function("swoole_event_wait", sizeof("swoole_event_wait")-1, &shutdown_function_entry TSRMLS_CC))
    {
        ...
        swWarn("Unable to register shutdown function [swoole_event_wait]");
    }
#else
...
void php_swoole_client_free(zval *object, swClient *cli TSRMLS_DC)
{
    ...
    if (cli->keep)
    {
        ...
        {
            swWarn("delete from hashtable failed.");
        }
...
swClient* php_swoole_client_new(zval *object, char *host, int host_len, int port)
{
    ...
    if (type & SW_FLAG_KEEP)
    {
        ...
        if (find == NULL)
        {
            ...
            {
                swWarn("swoole_client_create_socket add to hashtable failed.");
            }
            goto create_socket;
        }
        else
...
static PHP_METHOD(swoole_client, connect)
{
...
#ifdef YZ_DEBUG
    swWarn("swoole_client::connect.[cli=%p, ip=%s, port=%d, fd=%d, async=%d, keep=%d]", cli, host, port, cli->socket->fd, cli->async, cli->keep);
#endif
    ...
    {
        swWarn("swoole_client is already connected.");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_client, send)
{
    ...
    if (!cli)
    {
        swWarn("object is not instanceof swoole_client.");
        RETURN_FALSE;
    }

    if (!cli->socket) {
        swWarn("client socket is NULL.");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_client, getsockname)
{
    ...
    {
        swWarn("not connected to the server");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_client, close)
{
    ...
    if (!cli->socket)
    {
        swWarn("client not connected to the server");
        RETURN_FALSE;
    }
    ...
    int nPort;
    swClient_inet_peer(cli, &pHost, &nPort);
    swWarn("swoole_client::close [cli=%p, ip=%s, port=%d]", cli, pHost, nPort);
    sw_free(pHost)
#endif
