...
static int http_request_on_body(php_http_parser *parser, const char *at, size_t length)
{
    ...
    else if (ctx->mt_parser != NULL)
    {
        ...
        if (n != length)
        {
            swWarn("parse multipart body failed.");
        }
    }
...
static int http_onReceive(swServer *serv, swEventData *req)
{
    ...
    else
    {
        ...
        {
            swWarn("onRequest handler error");
        }
        if (EG(exception))
        {
...
#ifdef YZ_DEBUG
    swWarn("object_init_ex handle %d  11111", (int) Z_OBJ_HANDLE_P(zrequest_object));
#endif
...
#ifdef YZ_DEBUG
    swWarn("object_init_ex handle %d  22222", (int) Z_OBJ_HANDLE_P(zresponse_object));
#endif
...
static http_context* http_get_context(zval *object, int check_end TSRMLS_DC)
{
    ...
    if (!ctx)
    {
        swWarn("http client is not exist.");
        return NULL;
    }
...
{
#ifndef SW_HAVE_ZLIB
    swWarn("zlib library is not installed, cannot use gzip.");
    RETURN_FALSE;
#endif
    ...
    {
        swWarn("must use before send header.");
        RETURN_FALSE;
    }
