...
ZEND_BEGIN_ARG_INFO_EX(arginfo_zan_session_encode, 0, 0, 1)
ZEND_ARG_INFO(0, session_array)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_zan_session_decode, 0, 0, 1)
ZEND_ARG_INFO(0, data)
ZEND_END_ARG_INFO()

//for object style

//ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_server_nova_config, 0, 0, 2)
//    ZEND_ARG_OBJ_INFO(0, zobject, swoole_server, 0)
//    ZEND_ARG_INFO(0, zconfig)
//ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_server_deny_request_oo, 0, 0, 1)
...
const zend_function_entry swoole_functions[] =
{
    ...
    /*------nova_packet------*/
    PHP_FE(zan_session_encode, arginfo_zan_session_encode)
    PHP_FE(zan_session_decode, arginfo_zan_session_decode)
...
static zend_function_entry swoole_server_methods[] = {
    ...
    PHP_ME(swoole_server, deny_request, arginfo_swoole_server_deny_request_oo, ZEND_ACC_PUBLIC)
...
void swoole_set_object(zval *object, void *ptr)
{
    ...
    if (handle >= swoole_objects.size)
    {
...
#ifdef YZ_DEBUG
        swWarn("swoole_objects's size is not enough. [old_size=%d, new_size=%d, old_ptr=%p]", old_size, new_size, old_ptr);
#endif
        if (new_size > SWOOLE_OBJECT_MAX)
        {

#ifdef YZ_DEBUG
            swWarn("swoole_objects's new size is bigger than %d. [old_size=%d, new_size=%d, old_ptr=%p]", SWOOLE_OBJECT_MAX, old_size, new_size, old_ptr);
#endif
        ...
        {
#ifdef YZ_DEBUG
            swWarn("swoole_objects's new ptr is invalid. [old_size=%d, new_size=%d, old_ptr=%p]", old_size, new_size, old_ptr);
#endif
            return;
        }
...
PHP_MINIT_FUNCTION(swoole)
{
    ...
    swoole_aerospike_init(module_number TSRMLS_CC);
#endif
...
#endif
#ifdef SW_USE_AEROSPIKE
    php_info_print_table_row(2, "async aerospike client", "enabled");
#endif
...
PHP_FUNCTION(zan_session_encode)
{
    zval* session_array;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a", &session_array) == FAILURE) {
        RETURN_FALSE;
    }
    smart_str buf = {0};
    char *key;
    ...
    HashTable *_ht = Z_ARRVAL_P(session_array);
    ...
    for (zend_hash_internal_pointer_reset(_ht);
        ...
        if (zend_hash_get_current_data(_ht, (void**)&struc) == SUCCESS) {
            key_length --;
    ...
    smart_str_0(&buf);
    PHP_VAR_SERIALIZE_DESTROY(var_hash);
    SW_RETVAL_STRINGL(buf.c, buf.len, 0);
}

PHP_FUNCTION(zan_session_decode)
{
    char* pData;
    int nLen;

    if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &pData, &nLen) == FAILURE) {
        RETURN_FALSE;
    }
    ...
    while (p < endptr) {
        ...
        if (has_value) {
            ...
            if (php_var_unserialize(&current, (const unsigned char **) &q, (const unsigned char *) endptr, &var_hash TSRMLS_CC)) {
                zend_set_hash_symbol(current, name, namelen, PZVAL_IS_REF(current), 1, Z_ARRVAL_P(return_value));
            } else {
