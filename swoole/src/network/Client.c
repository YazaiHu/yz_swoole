...
int swClient_inet_peer(swClient* cli, char** host, int* port) {
    *host = sw_malloc(64);
    
    if (!cli)
    {
        swWarn("swClient::swClient_inet_peer: cli is null.");
        return SW_ERR;
    }
    
    if (!cli->socket) {
        swWarn("swClient::swClient_inet_peer: client socket is NULL.");
        return SW_ERR;
    }
    
    if (cli->type == SW_SOCK_UDP6 || cli->type == SW_SOCK_TCP6)
    {
        *port =  ntohs(cli->server_addr.addr.inet_v6.sin6_port);
        char tmp[INET6_ADDRSTRLEN];
        if (inet_ntop(AF_INET6, &cli->server_addr.addr.inet_v6.sin6_addr, tmp, sizeof(tmp)))
        {
            int nLen = snprintf(*host, 64, "%s", tmp);
            (*host)[nLen] = 0;
        }
        else
        {
            swWarn("swClient::swClient_inet_peer: inet_ntop() failed.");
        }
    }
    else
    {
        int nLen = snprintf(*host, 64, "%s", inet_ntoa(cli->server_addr.addr.inet_v4.sin_addr));
        (*host)[nLen] = 0;
        *port = ntohs(cli->server_addr.addr.inet_v4.sin_port);
    }
    return SW_OK;
}
#endif
...
//    swInfo("swClient close fd: %d.", cli->socket->fd);
    assert(fd != 0);
#ifdef SW_USE_OPENSSL
    ...
    if (cli->async)
    {
        ...
        if (cli->socket->active && cli->onClose)
        {
            ...
            swWarn("swClient_close async not onClose(), client socket addr: %p", cli->socket);
        }
    }
    else
    {
