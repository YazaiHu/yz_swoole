...
PHP_FUNCTION(swoole_mysql_query)
{
    ...
    if (sql.length <= 0)
    {
        swWarn("Query is empty.");
        RETURN_FALSE;
    }

    zend_class_entry *class_entry = zend_get_class_entry(mysql_link TSRMLS_CC);

    zval* connected = sw_zend_read_property(class_entry, mysql_link, ZEND_STRL("_connected"), 1 TSRMLS_CC);
    if (connected != NULL && !ZVAL_IS_NULL(connected)) {
        convert_to_boolean(connected);
        if(Z_BVAL_P(connected) == 0) {
            swWarn("Mysqli connection is lost.");
            RETURN_FALSE;
        }
    }
    int sock = -1;
    ...
    //    swWarn("swoole_mysql_query client socket: %d, mysql_link=%p.", sock, mysql_link);
    ...
    {
        swWarn("mysql client is waiting response, cannot send new sql query.");
        RETURN_FALSE;
    }
    ...
    if (SwooleG.main_reactor->write(SwooleG.main_reactor, sock, mysql_request_buffer->str, mysql_request_buffer->length) < 0)
    {
        ...
        if (swConnection_error(errno) == SW_CLOSE)
        {
            ...
            swWarn("swoole_mysql_query 1111 client addr NULL, mysql_link: %p, mysql_link_object: %d.", mysql_link, (int)Z_OBJ_HANDLE(*mysql_link));
...
static int swoole_mysql_onRead(swReactor *reactor, swEvent *event)
{
    ...
    mysql_client *client = swoole_get_object(mysql_link);
    if (!client) {
        swWarn("client addr is NULL, mysql_link: %p, mysql_link_object: %d", mysql_link, (int)Z_OBJ_HANDLE(*mysql_link));
        return SW_ERR;
    }
#ifdef YZ_DEBUG
    swWarn("swoole_mysql_onRead client addr: %p, mysql_link: %p, mysql_link_object: %d.", client, mysql_link, (int)Z_OBJ_HANDLE(*mysql_link));
#endif
    ...
    while(1)
    {
        ...
        close_fd:
            ...
            swoole_set_object(mysql_link, NULL);
            swWarn("swoole_mysql_onRead recv close, fd: %d.", event->fd);
            ...
            if (client->callback)
            {
                ...
                {
                    swWarn("swoole_async_mysql callback[2] handler error.");
                }
                if (retval)
                {
