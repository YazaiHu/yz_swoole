...
//    swWarn("###swoole redis.[redis=%p, obj=%p]", redis, getThis());
}
...
//    swWarn("###connect. [redis=%p, host=%s, port=%lu, callback=%p]", redis, host, port, callback);
    if (context->err)
    {
...
{
    swRedisClient *redis = swoole_get_object(getThis());
//    swWarn("###close. [redis=%p]", redis);
    if (redis && redis->context)
    {
        if (redis->connecting)
        {
            swWarn("redis client is connecting, cannot close.");
            RETURN_FALSE;
        }
...
static PHP_METHOD(swoole_redis, __call)
{
    ...
    swRedisClient *redis = swoole_get_object(getThis());
//    swWarn("###call. [redis=%p, command=%s, state=%lu]", redis, command, redis->state);
    if (!redis)
    {
        swWarn("object is not instanceof swoole_redis.");
        RETURN_FALSE;
    }
...
//    swWarn("###onResult. [redis=%p, callback=%p, type=%s]", redis, callback, callback_type);
    zval **args[2];
    args[0] = &redis->object;
    args[1] = &result;
    ...
    {
        swWarn("swoole_redis callback[%s] handler error.", callback_type);
    }
    if (EG(exception))
    {
...
//    swWarn("###onConnect. [redis=%p, callback=%p]", redis, zcallback);
    args[0] = &redis->object;
    args[1] = &result;
...
//    swWarn("###onClose. [redis=%p, callback=%p]", redis, zcallback);
...
static int swoole_redis_onError(swReactor *reactor, swEvent *event)
{
    ...
    zval *zcallback = sw_zend_read_property(swoole_redis_class_entry_ptr, redis->object, ZEND_STRL("onConnect"), 0 TSRMLS_CC);
//    swWarn("###onError. [redis=%p, callback=%p]", redis, zcallback);
