...
static PHP_METHOD(swoole_process, signal)
{
    ...
    if (!sw_zend_is_callable(callback, 0, &func_name TSRMLS_CC))
    {
        swWarn("function '%s' is not callable", func_name);
        efree(func_name);
        RETURN_FALSE;
    }
    efree(func_name);
...
static PHP_METHOD(swoole_process, read)
{
    ...
    if (process->pipe == 0)
    {
        swWarn("have not pipe, can not use read()");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_process, push)
{
    ...
    if (!process->queue)
    {
        swWarn("have not msgqueue, can not use push()");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_process, setaffinity)
{
    ...
    {
        swWarn("More than the number of CPU");
        RETURN_FALSE;
    }
...
static PHP_METHOD(swoole_process, close)
{
    ...
    if (process->pipe == 0)
    {
        swWarn("have not pipe, can not use close()");
        RETURN_FALSE;
    }
