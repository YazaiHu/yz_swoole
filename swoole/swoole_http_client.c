...
static int http_client_execute(zval *zobject, char *uri, zend_size_t uri_len, zval *callback TSRMLS_DC)
{
    ...
    if (http)
    {
        ...
        if (http->state != HTTP_CLIENT_STATE_READY)
        {
            ...
            swWarn("Operation now in progress phase %d.", http->state);
            return SW_ERR;
        }
        else if (!http->cli->socket || !http->cli->socket->active)
        {
            swWarn("connection#%d is closed.", http->cli->socket->fd);
            return SW_ERR;
        }
    }
    else
    ...
    {
        swWarn("swoole_http_client is already connected.");
        return SW_ERR;
    }
...
static void http_client_onClose(swClient *cli)
{
    ...
    if (!cli->socket) {
        swWarn("http_client_onClose socket is NULL .");
    }
    if (!cli->released)
    {
...
static void http_client_onReceive(swClient *cli, char *data, uint32_t length)
{
    ...
    http_client *http = swoole_get_object(zobject);
    if (!http) {
        swWarn("http_client is NULL");
        return;
    }
    if (!http->cli)
    {
        swWarn("object is not instanceof swoole_http_client.");
        return;
    }
    if (!http->cli->socket)
    {
        swWarn("http client socket is NULL");
        return;
    }
...
static PHP_METHOD(swoole_http_client, close)
{
    ...
    if (!cli)
    {
        swWarn("object is not instance of swoole_http_client.");
        RETURN_FALSE;
    }
    if (!cli->socket)
    {
        swWarn("http client not connected to the server");
        RETURN_FALSE;
    }
    if (cli->socket->closed)
    {
        //swWarn("http client socket is closed.");
        RETURN_FALSE;
    }
    int ret = SW_OK;
...
static int http_client_parser_on_message_complete(php_http_parser *parser)
{
    ...
    {
        swWarn("swoole_http_client object have not receive callback.");
    }
    char *func_name = NULL;
    if (!sw_zend_is_callable(zcallback, 0, &func_name TSRMLS_CC))
    {
        swWarn("swoole_http_cient object callback  function '%s' is not callable.", func_name);
        //efree(func_name);
    }
    efree(func_name);
