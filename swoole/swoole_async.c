...
static void php_swoole_aio_onComplete(swAio_event *event)
{
    ...
    if (event->type == SW_AIO_DNS_LOOKUP)
    {
        ...
        {
            swWarn("swoole_async: onAsyncComplete callback not found[0]");
            return;
        }
    ...
    else
    {
        ...
        if (!file_req)
        {
            swWarn("swoole_async: onAsyncComplete callback not found[1]");
            return;
        }
        if (file_req->callback == NULL && file_req->type == SW_AIO_READ)
        {
            swWarn("swoole_async: onAsyncComplete callback not found[2]");
            return;
        }
    ...
    if (ret < 0)
    {
        swWarn("swoole_async: Aio Error: %s[%d]", strerror(event->error), event->error);
    }
...
PHP_FUNCTION(swoole_async_read)
{
    ...
    if (offset < 0)
    {
        swWarn("offset must be greater than 0.");
        RETURN_FALSE;
    }
    ...
    {
        swWarn("offset must be less than file_size[=%lld].", file_stat.st_size);
        RETURN_FALSE;
    }
...
PHP_FUNCTION(swoole_async_write)
{
    ...
    if (file == NULL)
    {
        ...
        if (fd < 0)
        {
            swWarn("open file failed. Error: %s[%d]", strerror(errno), errno);
            RETURN_FALSE;
        }
...
PHP_FUNCTION(swoole_async_readfile)
{
    ...
    if (fd < 0)
    {
        swWarn("open file[%s] failed. Error: %s[%d]", Z_STRVAL_P(filename), strerror(errno), errno);
        RETURN_FALSE;
    }
    struct stat file_stat;
    if (fstat(fd, &file_stat) < 0)
    ...
    {
        swWarn("file_size[size=%ld|max_size=%d] is too big. Please use swoole_async_read.",
...
PHP_FUNCTION(swoole_async_writefile)
{
    ...
    {
        swWarn("file_size[size=%d|max_size=%d] is too big. Please use swoole_async_write.",
