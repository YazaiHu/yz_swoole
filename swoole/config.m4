...
PHP_ARG_ENABLE(youzan-debug, whether to enable youzan debug,
[  --enable-youzan-debug   Enable youzan debug], no, no)
...
PHP_ARG_ENABLE(aerospike, enable aerospike support,
[  --enable-aerospike    Do you have aerospike?], no, no)
...
if test "$PHP_SWOOLE" != "no"; then
    ...
    if test "$PHP_YOUZAN_DEBUG" != "no"; then
        AC_DEFINE(YZ_DEBUG, 1, [do we enable youzan debug])
    fi
    ...
    if test "$PHP_AEROSPIKE" = "yes"; then
        AC_DEFINE(SW_USE_AEROSPIKE, 1, [enable aerospike support])
    fi
    ...
    CFLAGS="-std=gnu99 -Wall -pthread $CFLAGS"
    ...
    if test "$PHP_AEROSPIKE" = "yes"; then
        ...
        case $os in
            Linux)
              #PHP_ADD_LIBRARY_WITH_PATH(aerospike, /opt/lib-aerospike-swoole/lib, SWOOLE_SHARED_LIBADD)
                PHP_ADD_LIBRARY_WITH_PATH(aerospike_linux, ../lib, SWOOLE_SHARED_LIBADD)
            ;;
            Darwin)
                PHP_ADD_LIBRARY_WITH_PATH(aerospike_mac, ../lib, SWOOLE_SHARED_LIBADD)
            ;;
        esac
    fi
    ...
    swoole_source_file="swoole.c \
        ...
        src/util/NetUtil.c \
        src/aerospike/sw_aerospike_event_loop.c \
        src/aerospike/sw_aerospike.c \
        swoole_aerospike.c"
    ...
    PHP_ADD_BUILD_DIR($ext_builddir/src/aerospike)
