...
int php_swoole_onReceive(swServer *serv, swEventData *req)
{
    ...
    {
        swWarn("php_swoole_onReceive function '%s' is not callable.", func_name);
        //efree(func_name);
        //RETURN_FALSE;
    }
    efree(func_name);
...
PHP_METHOD(swoole_server, __construct)
{
    ...
    if (serv_mode == SW_MODE_THREAD || serv_mode == SW_MODE_BASE)
    {
        ...
        swWarn("PHP can not running at multi-threading. Reset mode to SWOOLE_MODE_BASE");
    }
...
PHP_METHOD(swoole_server, addProcess)
{
    ...
    if (id < 0)
    {
        swWarn("swServer_add_worker failed.");
        RETURN_FALSE;
    }
...
PHP_METHOD(swoole_server, sendto)
{
    ...
    {
        swWarn("You must add an UDP6 listener to server before using sendto.");
        RETURN_FALSE;
    }
...
PHP_METHOD(swoole_server, deny_request)
{
    long nWorkerId;
    if (SwooleGS->start == 0)
    {
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &nWorkerId) == FAILURE)
    {
        return;
    }
...
{
    long nWorkerId = SwooleWG.id;
    zval *zobject = getThis();
    swServer *serv = swoole_get_object(zobject);
    swServer_tcp_deny_exit(serv, nWorkerId);
