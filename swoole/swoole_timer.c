...
static void swoole_onTimeout(swTimer* timer, swTimer_node *tnode) {
    if (1 == tnode->php_used) {
        php_swoole_onTimeout(timer, tnode);
    } else {
#ifdef SW_USE_AEROSPIKE
        asTimeCallBack* time_callback = (asTimeCallBack*)tnode->data;
        if (time_callback) {
            time_callback->cb(time_callback->data);
            sw_free(time_callback);
        }
        tnode->data = NULL;
#endif
    }
...
static void swoole_onInterval(swTimer* timer, swTimer_node* tnode) {
    if (1 == tnode->php_used) {
        php_swoole_onInterval(timer, tnode);
    } else {
#ifdef SW_USE_AEROSPIKE
        asTimeCallBack* time_callback = (asTimeCallBack*)tnode->data;
        if (time_callback) {
...
void php_swoole_check_timer(int msec)
{
    ...
    {
        swTimer_init(msec);
        SwooleG.timer.onAfter = swoole_onTimeout;
        SwooleG.timer.onTick = swoole_onInterval;
